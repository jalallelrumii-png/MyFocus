...
import { useTokens, fmt, Btn, Card, SectionLabel } from '../components/ui'
...
export function TimerPage() {
  const {
    ...
    darkMode, navigate, userName, toggleDarkMode,
  } = useStore()
    ...
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", background:T.bg, minHeight:'100vh', maxWidth:430, margin:'0 auto', paddingBottom:90, transition:'background 0.3s' }}>
      ...
      <div style={{ background:`linear-gradient(160deg,${T.cream} 0%,${T.bg} 100%)`, padding:'52px 22px 20px', borderBottom:`1px solid ${T.line}` }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div>
            <div style={{ fontSize:11, color:T.muted, letterSpacing:1 }}>{greeting}{displayName}</div>
          ...
          <div style={{ display:'flex', gap:6, alignItems:'center' }}>
            ...
            <button onClick={toggleDarkMode}
              style={{ width:34, height:34, borderRadius:10, background:T.tealBg, border:`1px solid ${T.tealLine}`, cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center' }}>
              {darkMode ? '☀️' : '🌙'}
            </button>
      ...
      <div style={{ padding:'20px 20px 0' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginBottom:20 }}>
          ...
          <div style={{ display:'flex', gap:12, marginTop:16, alignItems:'center' }}>
            ...
            <button onClick={()=>{ resumeAudio(); timerRunning ? pauseTimer() : startTimer() }}
